...
import com.hackerNews.Utilty.Utils;
import com.hackerNews.controller.HackerNewsApiController;
import com.hackerNews.exceptions.InternalServerException;
import com.hackerNews.exceptions.NoRecordFoundException;
...
import com.hackerNews.service.HnApiService;
...
public class HnApiServiceImpl implements HnApiService {
	 ...
	 private static final org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(HnApiServiceImpl.class);
	...
	public SortedSet<TopComment> getCommentsById(int storyId) {
		 ...
		 */
		SortedSet<TopComment> topCommentSet = new TreeSet<>(Comparator.comparing(TopComment::getTotalComments).reversed().thenComparing(TopComment::getTotalComments));
			...
			if (comments != null) {
				for (int commentId : comments) {
					...
					if (comment != null) {
						 ...
						 */
						TopComment topComment = new TopComment(comment.getText(), comment.getBy(),Utils.getUserAgeByName(comment.getBy()), commentCount);
