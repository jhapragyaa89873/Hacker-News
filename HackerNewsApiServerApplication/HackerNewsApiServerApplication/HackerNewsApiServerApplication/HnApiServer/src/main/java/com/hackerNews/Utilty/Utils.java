...
import com.hackerNews.exceptions.ConnectionException;
...
import com.hackerNews.model.User;
...
public class Utils {
	 ...
	 private static final org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(Utils.class);
	...
	public static int getUserAgeByName(String name) {
		...
		if (user != null) {
			return Period.between(
					Instant.ofEpochMilli(user.getCreated() * 1000L).atZone(ZoneId.systemDefault()).toLocalDate(),
			        LocalDate.now(ZoneId.systemDefault())).getYears();
		}
		return 0;
	}
